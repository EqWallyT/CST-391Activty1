    ...
    try {
        ...
        req.body.tracks.forEach(async (track: Track, index: number) => {
            try {
                ...
                res.status(500).json({
                    message: 'There was and error when writing album tracks'
                });
            }
        });;
        ...
        console.error('[albums.controller][createAlbums][Error] ', error);
        res.status(500).json({
            message: 'There was and error when writing albums'
        });
    }
};
export const updateAlbums: RequestHandler = async (req: Request, res: Response) => {
    try {
